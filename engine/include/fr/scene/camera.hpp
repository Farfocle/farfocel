...
/**
 * @brief Advances all FPS camera controllers by one frame.
 * @note Applies mouse look and WASD movement to every thing with FPSControllerPart +
 * WorldTransformPart.
 */
inline void update_fps_cameras(World &world, const WindowInput &input, float dt) noexcept {
    for (auto [thing, fps, trans] : world.query<FPSControllerPart, WorldTransformPart>()) {
        ...
        const glm::vec3 euler(glm::radians(fps.pitch), glm::radians(fps.yaw), 0.0f);
        trans.rotation = glm::quat(euler);

        const glm::vec3 forward = trans.rotation * glm::vec3(0.0f, 0.0f, -1.0f);
        const glm::vec3 right = trans.rotation * glm::vec3(1.0f, 0.0f, 0.0f);
        const glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f);
        const float speed = fps.move_speed * dt;
