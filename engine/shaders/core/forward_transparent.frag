...
const uint MATERIAL_BLEND_TRANSPARENT = 2u;
...
vec3 evaluate_specular_ibl(vec3 N, vec3 V, vec3 F0, float roughness, float ambient_ao) {
    ...
    float specular_occlusion = evaluate_specular_occlusion(ambient_ao, roughness, ndotv);
...
vec2 directional_poisson_offset(int index) {
    if (index == 0) return vec2(-0.942016, -0.399062);
    if (index == 1) return vec2(0.945586, -0.768907);
    if (index == 2) return vec2(-0.094184, -0.929389);
    if (index == 3) return vec2(0.344959, 0.293878);
    if (index == 4) return vec2(-0.915886, 0.457714);
    if (index == 5) return vec2(-0.815442, -0.879125);
    if (index == 6) return vec2(-0.382775, 0.276768);
    if (index == 7) return vec2(0.974844, 0.756484);
    if (index == 8) return vec2(0.443233, -0.975116);
    if (index == 9) return vec2(0.537430, -0.473734);
    if (index == 10) return vec2(-0.264969, -0.418930);
    if (index == 11) return vec2(0.791975, 0.190902);
    if (index == 12) return vec2(-0.241888, 0.997065);
    if (index == 13) return vec2(-0.814100, 0.914376);
    if (index == 14) return vec2(0.199841, 0.786414);
...
float calculate_shadow(vec3 world_pos, vec3 N, vec3 light_dir, float view_depth) {
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        visibility += texture(u_shadow_map, vec3(sample_uv, compare_depth));
    }
...
float sample_point_shadow_map(int shadow_index, vec3 direction) {
    if (shadow_index == 0) return texture(u_point_shadow_maps[0], direction).r;
    if (shadow_index == 1) return texture(u_point_shadow_maps[1], direction).r;
    if (shadow_index == 2) return texture(u_point_shadow_maps[2], direction).r;
    if (shadow_index == 3) return texture(u_point_shadow_maps[3], direction).r;
    return 1.0;
}
...
float calculate_point_shadow(PointLight light, vec3 world_pos) {
    ...
    float closest_depth = sample_point_shadow_map(light.shadow_index, light_to_frag);
    float visibility = (current_depth - bias <= closest_depth) ? 1.0 : 0.0;
...
vec3 evaluate_pbr_light(vec3 L, vec3 V, vec3 N, vec3 F0, vec3 albedo, float roughness,
                        float metallic, vec3 radiance) {
...
void main() {
    ...
    float view_depth =
        max(dot(v_world_pos - u_cam_pos.xyz, normalize(u_cam_forward.xyz)), 0.0);

    vec3 color_out = vec3(0.0);

    if (shading_model == 0u) {
        color_out = albedo * ambient_ao;
    } else if (shading_model == 1u) {
        ...
        if (num_dir_lights() > 0u) {
            ...
            float shadow = calculate_shadow(v_world_pos, N, L, view_depth);
        ...
        for (uint i = 0u; i < num_point_lights(); ++i) {
            ...
            if (dist < light.radius) {
                ...
                float shadow = calculate_point_shadow(light, v_world_pos);
                Lo += (albedo * diff + vec3(spec)) * radiance * (1.0 - shadow);
            }
        }
        ...
        for (uint i = 0u; i < num_spot_lights(); ++i) {
            ...
            vec3 L = spot_position(light) - v_world_pos;
            float dist = length(L);
            L = dist > 0.001 ? L / dist : vec3(0.0, 1.0, 0.0);
            ...
            if (dist < radius) {
                ...
                if (spot > 0.0) {
                    ...
                    float shadow = calculate_spot_shadow(light, v_world_pos);
                    Lo += (albedo * diff + vec3(spec)) * radiance * (1.0 - shadow);
                }
            }
        }
