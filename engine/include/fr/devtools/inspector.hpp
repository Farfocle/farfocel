...
inline void thing_parts_editor(World &world, Thing thing) noexcept {
...
/// @brief Draws the parts editor for a single thing using safe editor commands.
inline void thing_parts_editor(EditorContext &ctx, Thing thing) noexcept {
    ...
    if (ImGui::CollapsingHeader("Insert Part")) {
        ...
        for (USize i = 0; i < MAX_PARTS; ++i) {
            ...
            if (ImGui::SmallButton("Insert")) {
                insert_default_part(ctx, thing, tidx);
            }
...
inline void thing_explorer(World &world, InspectorState &state) noexcept {
...
inline void pool_explorer(World &world, InspectorState &state) noexcept {
...
inline void thing_explorer(EditorContext &ctx) noexcept {
    FR_ASSERT(ctx.is_valid(), "EditorContext must be valid");

    World &world = *ctx.world;
    DevToolsState &tools = *ctx.tools;
    InspectorState &state = tools.inspector;
...
inline void pool_explorer(EditorContext &ctx) noexcept {
    FR_ASSERT(ctx.is_valid(), "EditorContext must be valid");

    World &world = *ctx.world;
    InspectorState &state = ctx.tools->inspector;
...
inline void thing_inspector(EditorContext &ctx) noexcept {
    FR_ASSERT(ctx.is_valid(), "EditorContext must be valid");
    ...
    thing_parts_editor(ctx, t);
}

inline void pool_inspector(EditorContext &ctx) noexcept {
    FR_ASSERT(ctx.is_valid(), "EditorContext must be valid");
...
inline void thing_inspector(World &world, InspectorState &state) noexcept {
    Thing t = state.selected_thing;
...
inline void pool_inspector(World &world, InspectorState &state) noexcept {
    TypeIdx tidx = state.selected_pool;
    const PartMetaRegistry &meta = world.part_meta();
 ...
 */
inline void inspector(World &world, InspectorState &state) noexcept {
...
/**
 * @brief Draws the world inspector using safe editor commands.
 *
 * @param ctx Editor context with world, assets and shared devtools state.
 *
 * @note Must be called between `ImGui::Begin` / `ImGui::End`.
 */
inline void inspector(EditorContext &ctx) noexcept {
    FR_ASSERT(ctx.is_valid(), "EditorContext must be valid");

    const F32 left_w = 260.0f;

    ImGui::BeginChild("##insp_left", ImVec2(left_w, 0.0f), ImGuiChildFlags_Borders);
    impl::thing_explorer(ctx);
    ImGui::Spacing();
    impl::pool_explorer(ctx);
    ImGui::EndChild();
    ...
    switch (ctx.tools->inspector.active_view) {
    case InspectorState::View::Thing:
        impl::thing_inspector(ctx);
        break;

    case InspectorState::View::Pool:
        impl::pool_inspector(ctx);
        break;
